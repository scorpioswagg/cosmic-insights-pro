...
const Chart = z.object({
  ...
  ascendant: z.number(),
  midheaven: z.number(),
  bodies: z.array(Body).min(1).max(40),
...
const Input = z.object({
  ...
  charts: z.array(Chart).min(2).max(5),
});

function serial(c: z.infer<typeof Chart>): SerialChart {
  return {
    ...c,
...
export const generateMultiSynastryReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.charts.length < 2 || data.charts.length > 5) {
      throw new Error("Multi-chart synastry requires 2–5 charts.");
    }
...
Every chapter must open with an italic Chart Anchors line. Every substantive paragraph must cite supplied evidence. Use grounded language such as "this may suggest" and "this can manifest as". Chapter 30 must contain exactly five numbered Brutal Truths. Chapter 31 must end exactly with: Your chart does not give you an excuse. It gives you a mirror.`;
    ...
    const pairEvidence = group.pairs
      .map(
        (pair) =>
          `PAIR: ${pair.aName} ↔ ${pair.bName}\n${synastryEvidence(charts[pair.aIndex], charts[pair.bIndex], pair.result, def.title)}`,
      )
      .join("\n\n--- PAIR ---\n\n");
...
Rules: only this chapter; begin with one italic Chart Anchors line; every paragraph cites supplied evidence; do not invent data. If this is chapter 30, provide exactly five numbered Brutal Truths. If chapter 31, end with the required mirror sentence.`;
      ...
      try {
        ...
        if (statusCode === 402 || /payment required/i.test(message)) {
          console.error(
            `[generateMultiSynastryReport] AI balance unavailable for userId=${context.userId}, reportId=${data.reportId}`,
          );
          throw new Error(
            "Report writing is temporarily unavailable because the AI provider balance is exhausted. Set GEMINI_API_KEY to use your own Google billing, or top up Lovable credits.",
          );
        }
        throw error;
      }
